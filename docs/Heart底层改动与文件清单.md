...
```text
原生AI筛选 → 自动候选区 → 本人确认 → 冲突检查 → 原生A_memorix
手动 /记住 ──────────────────→ 冲突检查 → 原生A_memorix
原生查询/写入/删除 → Heart观察桥 → 记忆审计插件 → 中文日志
```
...
自动提取的人物事实默认先进入候选，确认候选或手动 `/记住` 后，before_memory_write 才检索本人旧事实并判断冲突。没有明确冲突时放行原生写入；不确定/失败时暂缓。明确冲突时生成受限原生计划、私聊原用户、持久化待确认请求。
