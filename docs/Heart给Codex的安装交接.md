...
- 日志位于实际 MaiBot 的 `data/heart_observation/logs/YYYY-MM-DD_群聊或私聊_名称_短编号.txt`（每天每会话一份，旧文件自动迁移，超过三天的整日审计记录清理），当前心情摘要位于 `data/heart_observation/当前心情.txt`。检查真实对话、记忆操作和心情变化原因，不把模型口头说“记住了”作为存储成功证据。
